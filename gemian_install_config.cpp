...
const char* language_codes[] = { "", "auto", "en_GB", "en_US", "en_IE", nullptr };

const char* language_items[] = {
  "Back",
  "Auto select from kb+tz",
  "Override to English (UK)",
  "Override to English (US)",
  "Override to English (IE)",
  nullptr
};
...
int index_for_language(std::string& language) {
  for (int i = 0; !language.empty() && language_codes[i] != nullptr; i++) {
    if (language.compare(language_codes[i]) == 0) {
      return i;
    }
  }
  return 0;  // auto
...
const char* name_for_language(std::string& language) {
  for (int i = 0; !language.empty() && language_codes[i] != nullptr; i++) {
    if (language.compare(language_codes[i]) == 0) {
      return language_items[i];
    }
  }
  return language_items[0];  // auto
}
